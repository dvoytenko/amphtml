...
export class ContextNode {
  ...
  /**
   * @param {*} id
   * @param {function(*, !ContextNode, !Function):ComponentImpl, !Array<ContextProp>} factory
   * @param {!Function} func
  ...
  setComponent(id, factory, func, deps, input) {
    ...
    if (!comp) {
      console.log('qqqq: new component created: ', id);
      // QQQQ: switch to factory from constructor.
      comp = factory(id, this, func, deps);
      components.set(id, comp);
    } else {
      console.log('qqqq: component found: ', id);
    }
    comp.set(input);
  }
  ...
  removeComponent(id) {
    ...
    if (comp) {
      console.log('qqqq: remove component: ', id);
      comp.dispose();
